...
bool CControlTester::Init(){
	if(!StartReadFile("data/launchData/controlTest.act")) return false;
...
EChangeModeFlag CControlTester::Process(){
	...
	case eStateMainStart:
		...
		if (!m_pGameStateManage->Init(*m_pGameDataManage)){
			DxLib::ErrorLogAdd(u8"制御テスト本体の初期化中にエラーが発生しました。");
			return eModeErrEnd;
		}
		break;
	case eStateErrEnd:
		DxLib::ErrorLogAdd(u8"制御テストの処理中にエラーが発生しました。");
		return eModeErrEnd;
	default:
	...
	if (!m_pGameStateManage->Draw(*m_pGameDataManage)){
		DxLib::ErrorLogAdd(u8"制御テスト画面描画中にエラーが発生しました。");
		return eModeErrEnd;
	}
