...
//		データチェック時にまとめて複製を行います。
	...
	for(unsigned int MainC=0;MainC<InputData.size();MainC++){
		...
		if (m_MainData[MainC].FileSize == 0){
			ErrorLogFmtAdd("IndexNo: %d, Name: \"%s\"が空か見つかりません。", MainC, nowReadFileName.c_str());
		}
		...
		case Copy:
			...
			break;
		default:
			ErrorLogFmtAdd("IndexNo.%dのType値が異常です。",MainC);
			Ans = false;
		}
	}
	...
	for(unsigned int MainC=0;MainC<m_MainData.size();MainC++){
		if(m_MainData[MainC].FileType!=Copy){
			...
			if(CheckHandleASyncLoad(Handle) == -1){
				DxLib::ErrorLogFmtAdd("IndexNo.%d,ファイル名\"%s\"の読み込みに失敗しました。",
			...
			if(ForkID<0||ForkID>=(signed)MainC){
				DxLib::ErrorLogFmtAdd("IndexNo.%dのCopy元IDが異常です。",MainC);
				Ans = false;
				break;
			}
			...
			case Sound:
				...
				if(m_MainData[MainC].DataHandle == -1){
					m_MainData[MainC].DataHandle = m_MainData[ForkID].DataHandle;
					DxLib::ErrorLogFmtAdd("IndexNo.%dのCopyに失敗しました。同じハンドルを渡します。",MainC);
				}
				break;
			case Material:
				m_MainData[MainC].DataHandle =
...
//		コピータイプが引数に指定された場合は消去に失敗したとみなす。
	if(DeletePos >= m_MainData.size()){
		const char* Err = "ファイル解放時に不正なハンドルを受け取りました。\n";
	...
	case Binary:
		...
		break;
	case Copy:
		const char* Err = "CopyフラグはDeleteDataでは解放できません。\n";
		DxLib::ErrorLogAdd(Err);
		return false;
		break;
	}
